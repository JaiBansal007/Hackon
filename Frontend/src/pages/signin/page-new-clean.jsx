  ...
  const handleSignIn = async (e) => {
    ...
    try {
      ...
      const result = isSignUp 
        ? await authService.createAccountWithEmail(email, password, recaptchaVerifier)
        : await authService.signInWithEmail(email, password, recaptchaVerifier)
      
      if (result.success) {
        console.log(`✅ Email ${isSignUp ? 'sign-up' : 'sign-in'} successful`)
        setSuccess(`${isSignUp ? 'Account created' : 'Sign-in'} successful! Redirecting...`)
        authService.clearRecaptcha()
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-8 relative">
        ...
        <div className="w-full max-w-md relative z-10">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-gray-600">Sign in to your FireStream account</p>
          </div>
            ...
            {authMethod === "google" && (
              <div>
                <Button
                  ...
                  disabled={isLoading}
                  className="w-full h-12 bg-white text-gray-700 border-2 border-gray-200 hover:bg-gray-50 hover:border-gray-300 font-semibold rounded-xl shadow-sm hover:shadow-md transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {isLoading ? (
              ...
              <div className="space-y-6">
                ...
                <form onSubmit={handleSignIn} className="space-y-6">
                  ...
                  <Button
                    ...
                    className="w-full h-12 bg-gradient-to-r from-yellow-400 to-orange-500 text-black hover:from-yellow-500 hover:to-orange-600 font-semibold rounded-xl shadow-lg hover:shadow-xl transform hover:scale-[1.02] transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none"
                  >
                    {isLoading ? (
                      <div className="flex items-center space-x-2">
                        <div className="w-5 h-5 border-2 border-gray-600 border-t-black rounded-full animate-spin"></div>

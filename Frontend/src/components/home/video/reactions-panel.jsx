    ...
    <AnimatePresence>
      {show && (
        <motion.div
          ...
          className="absolute bottom-24 left-1/2 transform -translate-x-1/2 bg-black/90 backdrop-blur-sm rounded-2xl p-4 flex items-center space-x-3 border border-gray-700/50 shadow-2xl"
        >
          {reactions.map((reaction, index) => (
            <motion.button
              ...
              className={`w-14 h-14 rounded-full bg-gray-800/80 hover:bg-gray-700/80 text-3xl p-0 border-2 border-transparent hover:border-orange-400/50 transition-all duration-200 backdrop-blur-sm ${
                isThrottled ? 'opacity-50 cursor-not-allowed' : 'hover:shadow-lg hover:shadow-orange-400/25'
              }`}
            >
              {reaction.emoji}
            </motion.button>
